// ============================================
// CALCUL STATS MENSUELLES - Version Améliorée avec support unités
// ============================================
...
// Fonction pour extraire la quantité d'un fruit (gère les deux formats)
function extractFruitQty(data) {
  // Nouveau format : {qty: 5, unite: 'piece'}
  if (typeof data === 'object' && data !== null && data.qty !== undefined) {
...
orders.forEach((order, index) => {
  ...
  // ✅ CORRECTION: Gérer le nouveau format avec {qty, unite}
  const normalizedFruits = {};
  Object.entries(fruitsComposition).forEach(([fruit, data]) => {
    const fruitKey = fruit.toLowerCase().trim();
    
    // Extraire la quantité (gère les deux formats)
    const qtyPerPanier = extractFruitQty(data);
    
    if (qtyPerPanier > 0) {
      normalizedFruits[fruitKey] = (normalizedFruits[fruitKey] || 0) + qtyPerPanier;
    }
  });
